  ...
  return (
    <div className="relative">
      <div className="absolute -left-5 -top-6 z-20 animate-floaty rounded-2xl border border-line bg-panel/90 px-4 py-3 shadow-card backdrop-blur md:-left-10">
        <div className="font-display text-[10px] font-bold uppercase tracking-widest text-muted">ROI médio saúde</div>
        <div className="font-display text-2xl font-bold text-neon">3.8x</div>
      </div>
      <div
      ...
      >
        <div className="font-display text-[10px] font-bold uppercase tracking-widest text-muted">Custo/lead médio</div>
        <div className="font-display text-2xl font-bold text-neon">R$ 12</div>
      </div>
      ...
      <div className="glass overflow-hidden p-6 shadow-card md:p-8">
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 font-display text-[10px] font-bold uppercase tracking-widest text-muted">
            <span className="h-2 w-2 animate-blink rounded-full bg-neon" /> Painel da clínica em tempo real
          </div>
        </div>

        <div className="mb-1 font-display text-sm font-medium text-muted">Pacientes novos este mês</div>
        <div className="mb-6 flex items-end gap-3">
          <span className="font-display text-6xl font-bold leading-none tracking-tight text-ink">+52</span>
          <span className="mb-1 rounded-full bg-neon/15 px-2.5 py-1 font-display text-xs font-bold text-neon">↑ 340%</span>
        </div>
      ...
      >
        ...
        <div>
          <div className="font-display text-sm font-bold text-ink">Consulta Agendada</div>
      ...
      >
        ...
        <div>
          <div className="font-display text-sm font-bold text-ink">Paciente Qualificado</div>
...
export default function Hero({ onOpenDiagnostico }: HeroProps) {
  const reduce = useReducedMotion();
  const up = (d: number) => ({
        ...
        <div>
          <motion.div {...up(0)}>
            <span className="chip">
              ...
              A Agência #1 de Marketing para Clínicas & Consultórios
            </span>
          </motion.div>
          ...
          <motion.p className="mt-7 max-w-lg text-lg leading-relaxed text-muted" {...up(0.16)}>
            A Cliqmed transforma anúncios no Meta e Google em consultas marcadas na sua recepção. Estratégia, conteúdo de autoridade e funil de captação direcionados por um único objetivo:{" "}
            <strong className="font-semibold text-ink">pacientes particulares na sua agenda.</strong>
          </motion.p>

          <motion.div className="mt-9 flex flex-wrap items-center gap-4" {...up(0.24)}>
            {onOpenDiagnostico ? (
              <button onClick={onOpenDiagnostico} className="btn-primary">
          ...
          <motion.ul className="mt-10 flex flex-wrap gap-x-7 gap-y-3 text-sm font-medium text-muted" {...up(0.32)}>
            {["Contrato sem fidelidade", "100% CFM/CFO Compliant", "1 clínica por especialidade na cidade"].map((t) => (
